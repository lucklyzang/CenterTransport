  ...
  methods: {
        ...
        .then((res) => {
          ...
          if (res && res.length > 0) {
            ...
            if (item4) {
              ...
              // 有暂存的话,则回显该暂存信息
              if (this.temporaryStorageCreateAppointTaskMessage['isTemporaryStorage']) {
                this.transportTypeList = this.temporaryStorageCreateAppointTaskMessage['transportTypeList']
              }
            }
          }
        })
    ...
    sureEvent () {
      ...
      let taskMessage = {
        ...
        toolId: this.currentTransportTool == '无工具' ? 0 : this.transportToolList.filter((item) => { return item.text == this.currentTransportTool })[0]['value'], //运送工具ID
      ...
      // 创建预约任务
      this.postGenerateAppointTask(taskMessage)
    },
